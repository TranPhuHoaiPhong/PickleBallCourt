...
function App() {
    ...
    const fetcDetailUser = async () => {
      ...
      try {
        if (token) {
          ...
          const { _id, name, email, isAdmin, phone } = res.data;
          dispatch(updateUser({ _id, name, email, isAdmin, phone }));
        }
      } catch (error) {
        console.log(error);
      }
  ...
  const clientId =
    "AffY-_4DDwR3-kTqn1_e1FYildsuD2lS9sqqTN65jrhASA3MWOg6YsMamsGFKPUhjlMMJ5o-uU-swFeJ";
